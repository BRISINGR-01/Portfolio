...
export default function ExperienceDisplay({ data }: { data: Experience | null }) {
	...
	return (
		<>
				...
				<G_Card position={Position.TopLeft} style={{ width: "min-content" }} className="col-2 m-4">
					<ChangeAnimation id={data.id} className="d-flex flex-column">
						<img
							...
							style={{ maxHeight: isICC(data) ? "60vh" : "20vh", maxWidth: "20vw", objectFit: "contain" }}
						/>
			...
			<motion.div
				key="description"
				style={{ top: 0, right: 0, position: "absolute", width: "inherit", zIndex: 10000 }}
				initial={{ transform: "translate(100px,-100px)", opacity: 0 }}
				animate={{ transform: "translate(0,0)", opacity: 1 }}
				exit={{ transform: "translate(100px,-100px)", opacity: 0 }}
				transition={TRANSITION}
...
function Description({ data }: { data: Experience }) {
	return (
						...
						<Accordion.Item eventKey="2">
							...
							<Accordion.Body className="d-flex gap-3">
								{Object.entries(data.links).map(([type, link]) => (
									<div key={link}>
										{/Github|LinkedIn/.test(link) && (
											<img
												src={`/icons/other/${link.includes("Github") ? "github" : "linkedin"}.svg`}
												style={{
													height: "1em",
													boxShadow: "inset rgb(255, 255, 255) 0px 0px 15px 2px",
													padding: "1px",
													marginRight: ".3em",
													borderRadius: link.includes("Github") ? "50%" : "2px",
												}}
											/>
										)}
...
function TechChart({ data }: { data: Experience }) {
	const icons = useIcon();

	if (!data.technologies) return <></>;

	return (
		<G_Card position={isICC(data) ? Position.BottomRight : Position.BottomLeft} className="m-4">
			<PieChart
				style={{
					maxHeight: "200px",
					maxWidth: "200px",
...
function isICC(data: Experience) {
	return data.id === "icc";
}
