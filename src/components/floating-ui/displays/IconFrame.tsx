...
export default function IconFrame(props: Props & { id: string }) {
	...
	switch (props.id) {
		case "asml":
			frame = "long-frame.png";
			break;
		case "sdg-solarwatt":
		case "sdg-sabic":
			frame = "circle-frame.png";
			break;
		default:
			frame = "square-frame.svg";
			break;
	}
		...
		>
			...
			>
				<ChangeAnimation id={props.id + "frame"} className="w-100 h-100">
					<img src={`/images/ui/${frame}`} alt="frame" width="100%" />
				</ChangeAnimation>
			</motion.div>
			<FadeAnim
			...
			>
				<ChangeAnimation id={props.id + "shadow"} className="w-100 h-100">
					<div
						...
						style={{
							background: "var(--dark)",
							borderRadius: frame === "circle-frame.png" ? "100%" : 0,
							display: frame === "long-frame.png" ? "none" : undefined,
						}}
					/>
				</ChangeAnimation>
			</FadeAnim>
