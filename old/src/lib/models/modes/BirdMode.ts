...
import type { ModeStrategy } from "../../utils/types";
...
export default class BirdMode implements ModeStrategy {
  start(character: Entity): Promise<boolean> {
    return wait(true);
  }

  stop(character: Entity): Promise<boolean> {
    return wait(true);
  }

  updateCamera(world: World, character: Entity) {
    const calcIdealOffset = () => {
      const idealOffset = thirdPerson
        ? new Vector3(0, camY, camZ)
        : new Vector3(0, 3, 7);
    ...
    const calcIdealLookat = () => {
      const idealLookat = thirdPerson
        ? new Vector3(0, -1.2, lookAtPosZ)
        : new Vector3(0, 0.5, lookAtPosZ + 5);
  ...
  render(world: World, character: Entity) {
